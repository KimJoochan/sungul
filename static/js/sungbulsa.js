(function($) {
  ...
  $('#nav').mouseenter(function(){
    var $gnb = $('#gnb');
    if(!$gnb.is(':animated')){$gnb.slideDown()}
  });
...
function login(){
  ...
  if(id.length<1){alert('아이디를 입력해주세요');return false;}
  else if(password.length<1){alert('비밀번호를 입력해주세요');return false;}
  else{
    $.ajax({
      url:'../board/loginAction.php',
      dataType:'JSON',
      type:'POST',
...
function insertScholar(){
  ...
  if(year.length<1){alert('장학년도를 입력해주세요');return false;}
  else if(name.length<1){alert('학생성명을 입력해주세요');return false;}
  else if(degree.length<1){alert('학교분류를 입력해주세요');return false;}
  else if(school.length<1){alert('학교명을 입력해주세요');return false;}
  else if(grade.length<1){alert('학년을 입력해주세요');return false;}
  else if(local.length<1){alert('지역을 입력해주세요');return false;}
  else{
    $.ajax({
      url:'../info/scholarAction.php',
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'year':year,'name':name,'degree':degree,'school':school,'grade':grade,'local':local},
      error:function(request,status,error){
         console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
      },
      success:function(data){
...
function updateScholar(idx){
  ...
  if(year.length<1){alert('장학년도를 입력해주세요');return false;}
    ...
    $.ajax({
      ...
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'year':year,'name':name,'degree':degree,'school':school,
      'grade':grade,'local':local,'idx':idx},
      error:function(request,status,error){
         console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
      },
      success:function(data){
      ...
      function updateScholarCnt(){
        ...
        var type = 'update_cnt';
        var year = $('#year').val();
        if(grade1.length<1||grade2.length<1||grade3.length<1||grade4.length<1){alert('인원수를 제대로 입력해주세요');return false;}
        else{
          $.ajax({
            url:'../info/scholarAction.php',
            data:{'grade1':grade1,'grade2':grade2,'grade3':grade3,'grade4':grade4,'type':type,'year':year},
            dataType:'JSON',
            type:'POST',
            error:function(request,status,error){
               console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
...
function insertEvent(){
  ...
  if(start.length<1){alert('시작일을 입력해주세요');return false;}
  else if(end.length<1){alert('마지막일을 입력해주세요');return false;}
  else if(title.length<1){alert('행사명을 입력해주세요');return false;}
  else{
    $.ajax({
      url:'../info/eventAction.php',
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'start':start,'end':end,'title':title,
...
function insertNotice(){
  ...
  var contents = $('#insertNotice-form .contents').val();

  if(title.length<1){alert('제목을 입력해주세요');return false;}
  else if(contents.length<1){alert('내용을 입력해주세요');return false;}
  else{ 
...
function deleteNotice(idx){
  if(confirm('정말 삭제하시겠습니까?')){
    ...
    $.ajax({
      ...
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'idx':idx,'file':file},
      error:function(request,status,error){
      },
      success:function(data){
        if(data.result=='1'){
...
function notice_search(){
  ...
  location.href='../board/notice.php?search='+search;
}
/*알림방 검색*/
...
function insertGallery(){
  ...
  var contents = $('#insertGallery-form .contents').val();
    ...
    $.ajax({
      ...
      success:function(data){
        if(data.result=='1'){
          ...
          location.href="../board/gallery.php";
          console.log('result : '+data.result+', msg : '+data.msg+
            ', msg2 : '+data.msg2);
        }else if(data.result=='0'){
          alert('오류가 발생하였습니다.');
...
function gallery_search(){
  ...
  location.href='../board/gallery.php?search='+search;
}
/*갤러리 검색*/
...
function updateGallery(idx){
  var type = 'update';
  var oldFile = $('#updateGallery-form .old-file').val();
  var title = $('#updateGallery-form .title').val();
  var contents = $('#updateGallery-form .contents').val();
...
function insertExecutive(){
  ...
  if(job.length<1){alert('직무를 입력해주세요');return false;}
  else if(name.length<1){alert('성명을 입력해주세요');return false;}
  else if(phone.length<1){alert('연락처를 입력해주세요');return false;}
  else{
    $.ajax({
      url:'../info/executiveAction.php',
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'job':job,'name':name,'phone':phone},
      error:function(request,status,error){
         console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
      },
      success:function(data){
...
function updateExecutive(idx){
  ...
  if(job.length<1){alert('직무를 입력해주세요');return false;}
    ...
    $.ajax({
      ...
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'job':job,'name':name,'phone':phone,'idx':idx},
      error:function(request,status,error){
         console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
      },
      success:function(data){
...
function moveExecutive(way,seq,test,idx){
    ...
    if(way=='up'&&seq==test){
      ...
      $.ajax({
        ...
        dataType:'JSON',
        type:'POST',
        data:{'type':type,'seq':seq,'idx':idx},
        error:function(request,status,error){
           console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
        },
        success:function(data){
...
function insertSchedule(period){
  ...
  if(title.length<1){alert('제목을 입력해주세요');return false;}
    ...
    $.ajax({
      ...
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'title':title,'contents':contents,'period':period},
      error:function(request,status,error){
         console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
      },
      success:function(data){
...
function deleteSchedule(period,idx){
  if(confirm('정말 삭제하시겠습니까?')){
    ...
    $.ajax({
      ...
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'idx':idx,'period':period},
      error:function(request,status,error){
         console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
      },
      success:function(data){
...
function updateSchedule(period,idx){
  ...
  if(title.length<1){alert('제목을 입력해주세요');return false;}
    ...
    $.ajax({
      ...
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'title':title,'contents':contents,'period':period,'idx':idx},
      error:function(request,status,error){
         console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
      },
      success:function(data){
...
function insertSponsor(){
  ...
  if(money.length<1){alert('후원금액을 입력해주세요');return false;}
  else if(name.length<1){alert('성명을 입력해주세요');return false;}
  else if(location.length<1){alert('주소를 입력해주세요');return false;}
  else{
    $.ajax({
      url:'../info/sponsorAction.php',
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'money':money,'name':name,'location':location},
      error:function(request,status,error){
         console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
      },
      success:function(data){
...
function updateSponsor(idx){
  ...
  if(money.length<1){alert('후원금액을 입력해주세요');return false;}
    ...
    $.ajax({
      ...
      dataType:'JSON',
      type:'POST',
      data:{'type':type,'money':money,'name':name,'location':location,'idx':idx},
      error:function(request,status,error){
         console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
      },
      success:function(data){
